...
do
    ...
    -- [[ Helper Functions from Silly.lua ]]
    local function create_mobile_button(name, position_y, color)
        ...
        bg.BackgroundColor3 = Color3.fromRGB(30, 30, 30) -- Default Theme.Container
        bg.Parent = button
        ...
        stroke.Color = Color3.fromRGB(160, 30, 255) -- Default Theme.Accent
        stroke.Thickness = 1
        ...
        text.TextSize = 14
        text.TextColor3 = Color3.fromRGB(255, 255, 255) -- Default Theme.Text
        text.ZIndex = 3
        text.Parent = button
    ...
    -- [[ System Struct for Compatibility ]]
    local SpamSystem = {
        __properties = {
    ...
    function SpamSystem.manual_spam.stop()
    ...
    local ManualSpam = rage:create_module({
        ...
        callback = function(state)
            if SpamSystem.__properties.__is_mobile then
                if state then
                    if not SpamSystem.__properties.__mobile_guis.manual_spam then
                        local manual_spam_mobile = create_mobile_button('Spam', 0.8, Color3.fromRGB(255, 255, 255))
                        SpamSystem.__properties.__mobile_guis.manual_spam = manual_spam_mobile
                        ...
                        manual_spam_mobile.button.InputEnded:Connect(function(input)
                            if input.UserInputType == Enum.UserInputType.Touch and not manual_was_dragged then
                                SpamSystem.__properties.__manual_spam_enabled = not SpamSystem.__properties.__manual_spam_enabled
                                
                                if SpamSystem.__properties.__manual_spam_enabled then
                                    SpamSystem.manual_spam.start()
                                    manual_spam_mobile.text.Text = "ON"
                                    manual_spam_mobile.text.TextColor3 = Color3.fromRGB(0, 255, 100)
                                else
                                    SpamSystem.manual_spam.stop()
                                ...
                                if getgenv().ManualSpamNotify then
                                    Library.SendNotification({
                                        title = "ManualSpam",
                                        text = SpamSystem.__properties.__manual_spam_enabled and "ON" or "OFF",
                                        duration = 2
                                    })
                                end
                            end
                    ...
                    SpamSystem.__properties.__manual_spam_enabled = false
                    SpamSystem.manual_spam.stop()
                    destroy_mobile_gui(SpamSystem.__properties.__mobile_guis.manual_spam)
                    SpamSystem.__properties.__mobile_guis.manual_spam = nil
                end
            else
                SpamSystem.__properties.__manual_spam_enabled = state
                if state then
                    SpamSystem.manual_spam.start()
                else
                    SpamSystem.manual_spam.stop()
    ...
    ManualSpam:create_slider({
        ...
        round_number = true,
        callback = function(value)
            SpamSystem.__properties.__spam_rate = value
        end
    })
