...
local System = {
    __properties = {
        ...
        __manual_spam_enabled = false,
        __auto_spam_enabled = false,
        __performance_mode = true, -- Added for laggy phones
        __play_animation = false,
        __curve_mode = 1,
        __accuracy = 1,
        __divisor_multiplier = 1.1,
...
function System.ball.get()
    local cache = System.__properties.__cache
    if cache.ball and cache.ball.Parent and cache.ball:GetAttribute('realBall') then
        return cache.ball
    end
...
function System.player.get_closest()
    ...
    if cache.closest_entity and (now - cache.last_player_update) < 0.1 then
        return cache.closest_entity
    end
    ...
    for _, entity in ipairs(Alive:GetChildren()) do
        if entity ~= LocalPlayer.Character and entity.PrimaryPart then
            local distance = (myPos - entity.PrimaryPart.Position).Magnitude
...
function System.player.get_closest_to_cursor()
    ...
    local entries = Alive:GetChildren()
    for i = 1, #entries do
        local player = entries[i]
        if player ~= character and player.PrimaryPart then
            local direction = (player.PrimaryPart.Position - camera.CFrame.Position).Unit
...
function System.auto_spam.start()
    ...
    System.__properties.__connections.__auto_spam = RunService.PreSimulation:Connect(function()
        ...
        local myPos = LocalPlayer.Character.PrimaryPart.Position
        local ballPos = ball.Position
        local entPos = entity.PrimaryPart.Position
...
-- Ball Cache Management
local BallsFolder = workspace:FindFirstChild("Balls")
if BallsFolder then
    BallsFolder.ChildRemoved:Connect(function(child)
        if System.__properties.__cache.ball == child then
            System.__properties.__cache.ball = nil
        end
    end)
end
...
function System.autoparry.start()
    ...
    System.__properties.__connections.__autoparry = RunService.PreSimulation:Connect(function()
        local props = System.__properties
        if not props.__autoparry_enabled or not LocalPlayer.Character or not LocalPlayer.Character.PrimaryPart then
            return
        end
