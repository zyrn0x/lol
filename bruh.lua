...
-- Fonction pour appliquer l'exploit Thunder Dash
local function applyThunderDashExploit()
    ...
    local success, module = pcall(function()
        return require(thunderModule)
    end)
    
    if success and module then
        local originalCooldown = module.cooldown
        local originalReduction = module.cooldownReductionPerUpgrade
        ...
        return function()
            -- Restauration
            module.cooldown = originalCooldown
            module.cooldownReductionPerUpgrade = originalReduction
        end
    end
    return nil
end

-- Fonction pour appliquer l'exploit Continuity Zero
local function applyContinuityZeroExploit()
    ...
    if not hooked then
        originalFireServer = hookfunction(ContinuityZeroRemote.FireServer, function(self, ...)
            if AbilityExploit.__config.enabled and getgenv().ContinuityZeroExploit then
                local args = {...}
                -- Modifier les arguments si c'est le remote ciblé
                if self == ContinuityZeroRemote then
    ...
    return function()
        if hooked and originalFireServer then
            hookfunction(ContinuityZeroRemote.FireServer, originalFireServer)
            hooked = false
        end
    end
end
...
function AbilityExploit.toggle(state)
    AbilityExploit.__config.enabled = state
    
    if state then
        -- Appliquer les exploits activés
        if getgenv().ThunderDashNoCooldown then
            local cleanup = applyThunderDashExploit()
            if cleanup then
                table.insert(cleanupFunctions, cleanup)
            end
        end
        
        if getgenv().ContinuityZeroExploit then
            local cleanup = applyContinuityZeroExploit()
            if cleanup then
                table.insert(cleanupFunctions, cleanup)
            end
        end
    else
        -- Nettoyer tous les exploits
    ...
    if AbilityExploit.__config.notify then
        Library.SendNotification({
...
local abilityExploitModule = devJV:create_module({
    ...
    section = 'left',
    callback = AbilityExploit.toggle
})

abilityExploitModule:create_checkbox({
    title = 'Thunder Dash No Cooldown',
    flag = 'ThunderDashNoCooldown',
    callback = function(value)
        getgenv().ThunderDashNoCooldown = value
        
        if AbilityExploit.__config.enabled then
            -- Réappliquer l'état
            AbilityExploit.toggle(false)
            AbilityExploit.toggle(true)
        end
    end
})
...
abilityExploitModule:create_checkbox({
    title = 'Notify',
    flag = 'AbilityExploitNotify',
    callback = function(value)
        AbilityExploit.__config.notify = value
    end
})
